...
@Component({
    selector: 'app-bonus-adjust-m',
    templateUrl: './bonus-adjust-m.component.html',
    styleUrls: ['./bonus-adjust-m.component.css'],

})
export class BonusAdjustMComponent implements OnInit {
    @Input()
    modal_name;
    @Input()
    modal_info;
    ...
    initForm(data) {
        ...
        for (const i in data.yg_item) {
            ...
            if (data.yg_item[i].is_annual == 1 && data.is_annualed == 1) {
                this.root_form.get('change_form').get(i).disable();
            }
        }
        console.log(this.root_form.get('change_form'));
    }

    compileData(data) {
        const source_obj = {
            is_annualed: data.data.data[0].annualed,
            yg_item: data.data.data[0].column,
            bonus_item: data.data.meta.tableHeader.filter((val, index) => {
                return index > 0;
            })
        };
    ...
    submitModal() {
        ...
        if (this.root_form.valid) {
            for (let i in this.root_form.get('change_form').getRawValue()) {
                const tmp = this.root_form.get('change_form').getRawValue();
        ...
        if (this.bonus.valid) {
            ...
            this.httpService.myPost(url, JSON.stringify(this.submit_data))
                .subscribe((data) => {
                    this.assistModalService.openAssistModal('toast', '操作成功', () => {
                        this.toggleEdit();
                        const bonus_tmp = this.show_data.bonus_item;
